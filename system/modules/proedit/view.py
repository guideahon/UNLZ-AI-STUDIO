...
class ProEditView(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        self.btn_install = ctk.CTkButton(actions, text=self.tr("proedit_btn_install"), command=self.install_backend)
        self.btn_install.pack(side="left", padx=5)

        self.btn_uninstall = ctk.CTkButton(actions, text=self.tr("proedit_btn_uninstall"), command=self.uninstall_backend)
        self.btn_uninstall.pack(side="left", padx=5)

        self.btn_deps = ctk.CTkButton(actions, text=self.tr("proedit_btn_deps"), command=self.install_deps)
        self.btn_deps.pack(side="left", padx=5)

        self.btn_open = ctk.CTkButton(actions, text=self.tr("proedit_btn_open_folder"), command=self.open_backend_folder)
        ...
        ctk.CTkLabel(log_frame, text=self.tr("proedit_log_label")).grid(row=0, column=0, sticky="w", padx=10, pady=(10, 0))
    ...
    def install_backend(self):
        if self.backend_dir.exists():
            self.log(self.tr("proedit_msg_already_installed"))
            self.refresh_buttons()
            return
        if not self.check_git_available():
            messagebox.showwarning(self.tr("status_error"), self.tr("proedit_msg_git_missing"))
            return
        self.backend_dir.parent.mkdir(parents=True, exist_ok=True)
        self.log(self.tr("proedit_msg_installing"))
        self.run_process(["git", "clone", "--depth", "1", "https://github.com/iSEE-Laboratory/ProEdit", str(self.backend_dir)])
        self.refresh_buttons()

    def uninstall_backend(self):
        if not self.backend_dir.exists():
            self.log(self.tr("proedit_msg_not_installed"))
            self.refresh_buttons()
            return
        try:
            import shutil
            shutil.rmtree(self.backend_dir)
            self.log(self.tr("proedit_msg_uninstalled"))
    ...
    def install_deps(self):
        if not self.backend_dir.exists():
            messagebox.showwarning(self.tr("status_error"), self.tr("proedit_msg_not_installed"))
            return
        req_path = self.backend_dir / "requirements.txt"
        if not req_path.exists():
            messagebox.showwarning(self.tr("status_error"), self.tr("proedit_msg_requirements_missing"))
            return
        python_path = sys.executable.replace("pythonw.exe", "python.exe")
        self.log(self.tr("proedit_msg_installing_deps"))
        ...
        def worker():
            try:
                ...
                if process.returncode == 0:
                    self.safe_log(self.tr("proedit_msg_done"))
                else:
                    self.safe_log(self.tr("proedit_msg_failed").format(process.returncode))
            except Exception as exc:
